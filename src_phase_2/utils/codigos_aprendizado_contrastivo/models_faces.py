...
def convVGG16(freeze):
    vgg16 = models.vgg16(pretrained=True)
    weight = vgg16.features[0].weight
    vgg16.features[0] = torch.nn.Conv2d(1, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
    vgg16.features[0].weight = torch.nn.Parameter(torch.mean(weight, dim=1, keepdim=True))
    
    if freeze:
        for param in vgg16.parameters():
            param.requires_grad = False

    vgg16.avgpool = torch.nn.AdaptiveAvgPool2d(output_size=(1, 1))
...
def convResNet18(freeze):
    resnet18 = models.resnet18(pretrained=True)
    
    weight = resnet18.conv1.weight
    resnet18.conv1 = torch.nn.Conv2d(1, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
    resnet18.conv1.weight = torch.nn.Parameter(torch.mean(weight, dim=1, keepdim=True))
