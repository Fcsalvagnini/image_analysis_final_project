...
if __name__ == '__main__':
    ...
    for fold, (train_idx, valid_idx) in enumerate(split):
        ...
        if config['training']['scheduler'] == 'stepLr':
            scheduler = torch.optim.lr_scheduler.StepLR(optimizer, gamma=0.1, step_size=config['training']['epochs'] - 1)
        ...
        if config['training']['scheduler'] == "oneCycle":
            scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer=optimizer, pct_start=0.1, div_factor=25,
                                                        max_lr=config['training']['max_lr'], epochs=config['training']['epochs'],
