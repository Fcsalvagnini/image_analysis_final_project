...
trn_ds = SiameseNetworkDatasetTriplet(items=trainset, transform=trn_tfms)
val_ds = SiameseNetworkDatasetTriplet(items=validset, transform=val_tfms)
...
save_figure(training_loss, validation_loss, "Training and Validation Loss", "Epochs", "Loss", "loss_triplet_corel.pdf")

save_figure(training_acc, validation_acc, "Training and Validation Accuracy", "Epochs", "Accuracy", "accuracy_triplet_corel.pdf")

tot_dl = next(iter(tot_dl))
imgs, labels = [t.to(device) for t in tot_dl]

torch.save(model, 'model_triplet_corel.pth')

outputA, outputB, outputC = model(imgs, imgs, imgs)

print('Saving t-SNE projections')

save_tsne_projection(outputA, labels, 0, nclasses, "Projection of the input MLP layer", "tsne-1st-layer_triplet_corel.pdf")

save_tsne_projection(outputA, labels, 1, nclasses, "Projection of the first hidden MLP layer", "tsne-2nd-layer_triplet_corel.pdf")

save_tsne_projection(outputA, labels, 2, nclasses, "Projection of the second hidden MLP layer", "tsne-3rd-layer_triplet_corel.pdf")

save_tsne_projection(outputA, labels, 3, nclasses, "Projection of the output MLP layer", "tsne-4th-layer_triplet_corel.pdf")
