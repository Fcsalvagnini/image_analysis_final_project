...
trn_ds = SiameseNetworkDataset(items=trainset, transform=trn_tfms)
val_ds = SiameseNetworkDataset(items=validset, transform=val_tfms)
...
criterion = ContrastiveLossChopra()
...
for epoch in range(n_epochs):
    ...
    loss, acc = validate_batch(model, trn_batch_dl, criterion, device)

    training_loss.append(loss)
    training_acc.append(acc)

    val_batch_dl = next(iter(val_iter_dl))
    loss, acc = validate_batch(model, val_batch_dl, criterion, device)
...
save_figure(training_loss, validation_loss, "Training and Validation Loss", "Epochs", "Loss", "loss_contrastive_corel.pdf")

save_figure(training_acc, validation_acc, "Training and Validation Accuracy", "Epochs", "Accuracy", "accuracy_contrastive_corel.pdf")

tot_dl = next(iter(tot_dl))
imgs, labels = [t.to(device) for t in tot_dl]

torch.save(model, 'model_contrastive_corel.pth')

outputA, outputB = model(imgs, imgs)

print('Saving t-SNE projections')

save_tsne_projection(outputA, labels, 0, nclasses, "Projection of the input MLP layer", "tsne-1st-layer_contrastive_corel.pdf")

save_tsne_projection(outputA, labels, 1, nclasses, "Projection of the first hidden MLP layer", "tsne-2nd-layer_contrastive_corel.pdf")

save_tsne_projection(outputA, labels, 2, nclasses, "Projection of the second hidden MLP layer", "tsne-3rd-layer_contrastive_corel.pdf")

save_tsne_projection(outputA, labels, 3, nclasses, "Projection of the output MLP layer", "tsne-4th-layer_contrastive_corel.pdf")
